  ...
  const clr = () => { //function made to replace whole string in seText use state with blank
   ...
   const italic = () => { //function made to replace selected words to italic
      document.getElementById("exampleFormControlTextarea1").addEventListener('mouseup',(evt)=>{
         ...
         console.log(selt) //to-do fix this button and then put bold underline logic as well
     });
   };
  ...
  return (
    <div className="container-fluid" style={App.myStyle}>
      ...
      <div className="container mx-2 my-2">
        <button className="btn btn-outline-primary mx-1" onClick={upclick}>
          Uppercase
        </button>
        <button className="btn btn-outline-primary mx-1" onClick={dwclick}>
          Lowercase
        </button>
        <button className="btn btn-outline-primary mx-1" onClick={rmvspcs}>
          Remove spaces
        </button>
        <button
        ...
        <button className="btn btn-outline-primary mx-1" onClick={italic}>
          <i className="bi bi-type-italic"></i>
        </button>
        <button className="btn btn-outline-primary mx-1" onClick={clr}>
      ...
      <em className="text">
        {text.length} characters, {text.split(" ").length - 1} words, You can read
        it in {(text.split(" ").length - 1) * 0.008} mins
      </em>
    </div>
  );
}
